...
| フレーム | 説明 | 用途 |
|---------|------|-----|
| 3Eバイナリ | Qシリーズ標準 | 最も一般的 |
| 4Eバイナリ | iQ-Rシリーズ | シリアル番号付き |
| 1Eバイナリ | A互換/QnA互換 | レガシークライアント |
| 3E ASCII | テキストベース | デバッグ・ログ用 |
| 4E ASCII | テキストベース | デバッグ・ログ用 |

### 対応シリーズ
- **Qシリーズ**: 3Eフレーム対応
- **iQ-Rシリーズ**: 4Eフレーム対応
- **A互換**: 1Eフレーム対応（レガシークライアント向け）
...
├── gui.py              # 統合GUI（旧版）
├── test_plc_client.py  # プロトコルテストクライアント
└── README.md           # このファイル
```

## 外部PLCクライアントとの互換性

このモックPLCサーバーは、標準的なMCプロトコルをサポートする
任意のPLCクライアントアプリケーションから接続できます。

### 対応している一般的なライブラリ

- **pymcprotocol**: Python用MCプロトコルライブラリ
- **mcprotocol**: Node.js用MCプロトコルライブラリ
- **自作PLCクライアント**: 3E/4E/1E/ASCIIフレームに対応

### 動作確認用テスト

```bash
# テストクライアントで全フレームフォーマットをテスト
python test_plc_client.py
```

テストクライアントは以下のフレームフォーマットをテストします：
- 3Eバイナリ: ワード読み書き、ビット読み書き
- 1Eバイナリ: ワード読み書き
- 3E ASCII: ワード読み出し
